...
int no_of_phil_eating=0; //No. of philosopher currently eating
...
void check_deadlock()
{
	if(no_of_phil_eating>0)
		return;	//no deadlock if someone is no_of_phil_eating
	...
	//there might be allocation_matrix case that allocation_matrix Philosopher picked right fork just before deadlock was checking for busy forks, in this case, this function would consider that no one is no_of_phil_eating and all forks are busy, meaning deadlock, while actually there is no deadlock
...
void *dinner(void* p)
{
	...
	while(1)
	{
		...
		no_of_phil_eating++;	//inc. count of no_of_phil_eating after Philosopher no_of_phil_eatings
		printf("Philosopher %d starts eating.\n",pos);
		sleep(rand() % 2 + 1);		//Philosopher no_of_phil_eatings for at least 1 sec or at most 3 seconds
...
int main()
{
	...
	//crno_of_phil_eatinge threads
	for(int i=0;i<n;i++)
	{
		if(pthread_create(&tid[i],NULL,dinner,&ph[i]))
			printf("failed thread crno_of_phil_eatingion");
